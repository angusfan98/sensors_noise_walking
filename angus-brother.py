...
def cleanData(ax):
    b,a = signal.butter(4,0.1,btype='lowpass',analog=False)            #clean the data using butterworth filter
...
#MAIN CODE
df = pd.read_csv('angusbrother.txt',sep=';',index_col = 'index')              #read in the data
df = df[(df.index > 1700) & (df.index <3700)]                              #shave off entries that may not be periodic to un-bias the readings
...
new_row = {'name':'Barry','age':29,'gender':'male','height':180,'steps_min':stepsMin,'time':timeElapsed,'steps':numSteps,'pace':stepsMin}
