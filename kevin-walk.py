...
#MAIN CODE
df = pd.read_csv('kevinwalk.txt',sep=';',index_col = 'index')              #read in the data
df = df[(df.index > 24) & (df.index <2540)]                              #shave off entries that may not be periodic to un-bias the readings
...
new_row = {'name':'Kevin','age':22,'gender':'male','height':170,'steps_min':stepsMin,'time':timeElapsed,'steps':numSteps,'pace':stepsMin}
