...
#MAIN CODE
df = pd.read_csv('annawalk.txt',sep=';',index_col = 'index')              #read in the data
df = df[(df.index > 2400) & (df.index <4800)]                              #shave off entries that may not be periodic to un-bias the readings
...
new_row = {'name':'Anna','age':22,'gender':'female','height':167,'steps_min':stepsMin,'time':timeElapsed,'steps':numSteps,'pace':stepsMin}
