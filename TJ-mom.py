...
#MAIN CODE
df = pd.read_csv('TJMomwalk.txt',sep=';',index_col = 'index')              #read in the data
df = df[(df.index > 24) & (df.index <600)]                              #shave off entries that may not be periodic to un-bias the readings
...
new_row = {'name':'Hana','age':61,'gender':'female','height':152,'steps_min':stepsMin,'time':timeElapsed,'steps':numSteps,'pace':stepsMin}
